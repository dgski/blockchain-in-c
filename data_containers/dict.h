...
bt_node* bt_node_remove(bt_node* in_head, char* in_key); //Removes node properly from tree
...
int dict_del_elem(dict* in_dict, char* in_key); //Delete element with key
int dict_discard(dict* in_dict); //Delete entire dictionary 
void dict_print(dict* in_dict, int verbosity); //Print out dictionary
int dict_foreach(dict* in_dict, int (*func)(bt_node* current_node), void* data); //Run given function on all dict element
